        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-2 ml-5">
              Bem-vindo, <span className="text-white font-medium">{user?.nome || 'Gestor'}</span>. Sua oficina está rodando 100%.
            </p>
          </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 bg-white/5 rounded-lg border border-white/5">
                  ...
                  <div>
                    <p className="text-sm text-gray-300">5 revisões vencem essa semana.</p>
                    <button className="text-xs text-blue-400 hover:text-blue-300 mt-1 font-medium">Disparar WhatsApp</button>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 bg-white/5 rounded-lg border border-white/5">
                  <div className="w-2 h-2 mt-2 rounded-full bg-green-500 shrink-0" />
                  <div>
                    <p className="text-sm text-gray-300">Backup do banco realizado com sucesso.</p>
                    <span className="text-xs text-gray-600 mt-1 block">Há 2 horas</span>
                  </div>
                </div>
              </div>
            </div>
...
function NewServiceModal({ onClose, onSuccess, oficinaId }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Criar ou Buscar Cliente (Simplificado: Cria novo sempre para MVP, idealmente buscaria por placa/whatsapp)
      const { data: cliente, error: clienteError } = await supabase
        ...
        .insert({
          ...
          whatsapp: formData.clienteWhatsapp, // Usando mesmo campo para facilitar
          carro_modelo: formData.carroModelo,
          carro_ano: formData.carroAno,
      ...
      const { error: servicoError } = await supabase
        ...
        .insert({
          ...
          tipo_servico: formData.servicoTipo,
